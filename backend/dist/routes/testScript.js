...
// 获取所有测试脚本
router.get('/scripts', auth_1.authenticateToken, auth_1.requireAdmin, (req, res) => {
    try {
...
// 获取特定测试脚本
router.get('/scripts/:scriptId', auth_1.authenticateToken, auth_1.requireAdmin, (req, res) => {
    try {
        const { scriptId } = req.params;
        const script = testScriptService_1.testScriptService.getScript(scriptId);
        if (!script) {
...
// 创建测试脚本
router.post('/scripts', auth_1.authenticateToken, auth_1.requireAdmin, (req, res) => {
    try {
        ...
        const script = testScriptService_1.testScriptService.createScript({
            name,
...
// 启用/禁用测试脚本
router.patch('/scripts/:scriptId/toggle', auth_1.authenticateToken, auth_1.requireAdmin, (req, res) => {
    try {
        const { scriptId } = req.params;
        ...
        const success = testScriptService_1.testScriptService.toggleScript(scriptId, isActive);
        if (!success) {
            return res.status(404).json({
                success: false,
                message: '测试脚本不存在'
        ...
        res.json({
            ...
            data: testScriptService_1.testScriptService.getScript(scriptId)
        });
    }
    catch (error) {
...
// 删除测试脚本
router.delete('/scripts/:scriptId', auth_1.authenticateToken, auth_1.requireAdmin, (req, res) => {
    try {
        const { scriptId } = req.params;
        const success = testScriptService_1.testScriptService.deleteScript(scriptId);
        if (!success) {
            return res.status(404).json({
                success: false,
                message: '测试脚本不存在'
...
// 创建默认测试场景
router.post('/default-scenarios', auth_1.authenticateToken, auth_1.requireAdmin, (req, res) => {
    try {
        testScriptService_1.testScriptService.createDefaultTestScenarios();
        res.json({
            success: true,
            message: '默认测试场景创建成功',
            data: testScriptService_1.testScriptService.getAllScripts()
        });
    }
    catch (error) {
...
// 重置所有任务状态
router.post('/reset-tasks', auth_1.authenticateToken, auth_1.requireAdmin, (req, res) => {
    try {
        testScriptService_1.testScriptService.resetAllTasks();
        res.json({
            success: true,
            message: '所有测试任务状态已重置',
            data: testScriptService_1.testScriptService.getAllScripts()
